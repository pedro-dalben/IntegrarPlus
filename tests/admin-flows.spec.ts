...
test.describe('Admin Flows - Teste Completo', () => {
  ...
  test('Teste Completo - Especializações', async ({ page }) => {
    ...
    await page.fill('textarea[name="specialization[description]"]', 'Descrição da especialização teste');
  ...
  test('Teste Completo - Tipos de Contrato', async ({ page }) => {
    ...
    await page.fill('textarea[name="contract_type[description]"]', 'Descrição do tipo de contrato teste');
  ...
  test('Verificação de Layout e Consistência Visual', async ({ page }) => {
    ...
    for (const section of sections) {
      ...
      const novoBotao = page.locator('a').filter({ hasText: /Novo|Nova/ }).first();
