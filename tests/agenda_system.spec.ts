...
test.describe('Sistema de Agendas', () => {
  ...
  test('deve permitir vincular profissionais à agenda', async ({ page }) => {
    ...
    await expect(page.locator('[data-agendas-professionals-target="selectedList"]')).toContainText('Dr.');
  ...
  test('deve validar seleção de profissionais obrigatória', async ({ page }) => {
    ...
    await expect(page.locator('.ta-alert-warning')).toContainText('É necessário selecionar pelo menos um profissional');
  });
});
