...
export default class extends Controller {
  ...
  async exportThumbnailPNG() {
    return new Promise((resolve) => {
      ...
      this.sendMessage({ action: "export", format: "png", scale: 1, border: 5, embedImages: true, background: "#ffffff" })
      setTimeout(() => { window.removeEventListener("message", onMessage); resolve() }, 4000)
    })
  }
