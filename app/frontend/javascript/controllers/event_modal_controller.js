...
export default class extends Controller {
  ...
  populateForm(eventData) {
    ...
    form.querySelector('[name="event[start_time]"]').value = this.formatDateTimeForInput(
      eventData.start
    );
    form.querySelector('[name="event[end_time]"]').value = this.formatDateTimeForInput(
      eventData.end
    );
  ...
  async updateEvent() {
    ...
    const { eventId } = this.formTarget.dataset;
    if (!eventId) return;
