class Ui::ProTableComponent < ViewComponent::Base
  ...
  def thead
    content_tag :thead do
      content_tag :tr, class: "text-left text-gray-500 border-b", style: "border-color: rgb(var(--t-fg) / 0.06)" do
        safe_join(@columns.map { |c| content_tag(:th, c[:label], class: "py-2 pr-4") } + [ (@actions ? content_tag(:th, "", class: "py-2 pr-4 text-right") : nil) ].compact)
      end
    end
  end
