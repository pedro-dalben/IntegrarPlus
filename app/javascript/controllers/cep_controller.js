...
export default class extends Controller {
  ...
  search() {
    ...
    fetch(`${this.urlValue}/${zipCode}`)
      ...
      .catch(error => {
        ...
        this.showError('Error consulting ZIP code')
        this.clearFields()
      })
      .finally(() => {
  ...
  buscar() { this.search() }
  formatarCep() { this.formatZipCode() }
  preencherCampos(data) { this.fillFields(data) }
  limparCampos() { this.clearFields() }
  mostrarLoading(show) { this.showLoading(show) }
  desabilitarCampos(disable) { this.disableFields(disable) }
  mostrarErro(message) { this.showError(message) }
}
