class VersionCommentsController < ApplicationController
  ...
  before_action :ensure_can_edit_comment, only: [:update, :destroy]
  ...
  def create
    ...
    if @comment.save
      respond_to do |format|
        format.html { redirect_to document_path(@document_version.document), notice: 'Comentário adicionado com sucesso!' }
        format.json { render json: { success: true, comment: render_comment(@comment) } }
        format.turbo_stream { render turbo_stream: turbo_stream.append("comments-#{@document_version.id}", partial: 'version_comments/comment', locals: { comment: @comment }) }
      end
    else
      respond_to do |format|
        ...
        format.turbo_stream { render turbo_stream: turbo_stream.replace('comment-form', partial: 'version_comments/form', locals: { comment: @comment, document_version: @document_version }) }
      end
    end
  end
  ...
  def update
    if @comment.update(comment_params)
      respond_to do |format|
        format.html { redirect_to document_path(@document_version.document), notice: 'Comentário atualizado com sucesso!' }
        format.json { render json: { success: true, comment: render_comment(@comment) } }
        format.turbo_stream { render turbo_stream: turbo_stream.replace("comment-#{@comment.id}", partial: 'version_comments/comment', locals: { comment: @comment }) }
      end
    else
      respond_to do |format|
        format.html { redirect_to document_path(@document_version.document), alert: 'Erro ao atualizar comentário.' }
        format.json { render json: { error: @comment.errors.full_messages.join(', ') }, status: :unprocessable_entity }
        format.turbo_stream { render turbo_stream: turbo_stream.replace("comment-#{@comment.id}", partial: 'version_comments/comment_edit', locals: { comment: @comment }) }
      end
    end
  end
  ...
  def ensure_can_comment
    unless @document_version.document.user_can_comment?(current_user)
      respond_to do |format|
        format.html { redirect_to @document_version.document, alert: 'Você não tem permissão para comentar neste documento.' }
  ...
  def ensure_can_edit_comment
    unless @comment.can_be_edited_by?(current_user)
      respond_to do |format|
        format.html { redirect_to @document_version.document, alert: 'Você não tem permissão para editar este comentário.' }
        format.json { render json: { error: 'Acesso negado' }, status: :forbidden }
      end
    end
  end
