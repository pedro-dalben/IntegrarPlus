class Admin::AgendaDashboardController < Admin::BaseController
  ...
  def calculate_dashboard_metrics
    {
      ...
      agendas_with_conflicts: Agenda.active.select(&:has_conflicts?).count,
  ...
  def calculate_occupancy_trend
    current_month_occupancy = calculate_monthly_occupancy(Date.current.beginning_of_month..Date.current.end_of_month)
    last_month_occupancy = calculate_monthly_occupancy(1.month.ago.beginning_of_month..1.month.ago.end_of_month)
  ...
  def calculate_utilization_chart_data(professional_ids = nil)
    ...
    professionals.map do |professional|
      ...
      professional_slots = professional.agendas.active.sum { |agenda| agenda.calculate_total_slots(30.days.ago..Date.current) }
