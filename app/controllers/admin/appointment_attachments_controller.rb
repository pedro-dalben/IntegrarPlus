...
module Admin
  class AppointmentAttachmentsController < Admin::BaseController
    before_action :set_appointment_attachment, only: %i[show download destroy]
    ...
    def destroy
      if @appointment_attachment.can_be_deleted_by?(current_user)
        ...
        redirect_to admin_medical_appointment_path(@medical_appointment),
                    alert: 'Você não tem permissão para remover este anexo.'
      end
    end
    ...
    def appointment_attachment_params
      params.expect(appointment_attachment: %i[attachment_type name file])
    end
  end
end
