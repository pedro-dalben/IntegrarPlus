class Admin::MedicalAppointmentsController < Admin::BaseController
  before_action :set_medical_appointment, only: [:show, :edit, :update, :destroy, :cancel, :reschedule, :complete, :mark_no_show]
  ...
  def index
    ...
    @medical_appointments = @medical_appointments.where(professional: params[:professional]) if params[:professional].present?
    @medical_appointments = @medical_appointments.where(appointment_type: params[:appointment_type]) if params[:appointment_type].present?
  ...
  def update
    @medical_appointment = MedicalAppointmentService.update_appointment(@medical_appointment, medical_appointment_params)
  ...
  def cancel
    if @medical_appointment.can_be_cancelled?
      ...
      redirect_to admin_medical_appointment_path(@medical_appointment), alert: 'Não é possível cancelar esta consulta.'
    end
  end
  ...
  def reschedule
    if @medical_appointment.can_be_rescheduled?
      ...
      redirect_to admin_medical_appointment_path(@medical_appointment), alert: 'Não é possível reagendar esta consulta.'
    end
  end
  ...
  def mark_no_show
    ...
    redirect_to admin_medical_appointment_path(@medical_appointment), notice: 'Consulta marcada como não comparecimento.'
  end
  ...
  def medical_appointment_params
    params.require(:medical_appointment).permit(
      :agenda_id, :professional_id, :patient_id, :appointment_type, :status, :priority,
