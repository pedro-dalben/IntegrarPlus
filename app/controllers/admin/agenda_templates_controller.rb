...
module Admin
  class AgendaTemplatesController < Admin::BaseController
    ...
    def destroy
      if @template.usage_count.zero?
        ...
        redirect_to admin_agenda_template_path(@template),
                    alert: 'Não é possível excluir um template que já foi utilizado.'
      end
    end
    ...
    def template_params
      params.expect(
        agenda_template: [:name, :description, :category, :visibility,
                          { template_data: {} }]
      )
    end
