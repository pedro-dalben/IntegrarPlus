class Admin::AppointmentNotesController < Admin::BaseController
  before_action :set_appointment_note, only: [:show, :edit, :update, :destroy]
  ...
  def destroy
    if @appointment_note.can_be_deleted_by?(current_user)
      ...
      redirect_to admin_medical_appointment_path(@medical_appointment), alert: 'Você não tem permissão para remover esta anotação.'
    end
  end
  ...
  def appointment_note_params
    params.require(:appointment_note).permit(:note_type, :content)
  end
end
