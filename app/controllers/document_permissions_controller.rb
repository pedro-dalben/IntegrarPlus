class DocumentPermissionsController < ApplicationController
  ...
  def index
    ...
    @users = User.where.not(id: @document.author_id).order(:name)
    ...
    begin
      ...
      if grantee
        ...
        respond_to do |format|
          format.html { redirect_to document_document_permissions_path(@document), notice: 'Permissão concedida com sucesso!' }
          format.json { render json: { success: true, message: 'Permissão concedida com sucesso!' } }
        end
      else
        respond_to do |format|
          format.html { redirect_to document_document_permissions_path(@document), alert: 'Usuário ou grupo não encontrado.' }
          format.json { render json: { error: 'Usuário ou grupo não encontrado.' }, status: :not_found }
        end
      end
    rescue ActiveRecord::RecordInvalid => e
      respond_to do |format|
        format.html { redirect_to document_document_permissions_path(@document), alert: e.message }
  ...
  def destroy
    ...
    if @document.revoke_permission(grantee)
      respond_to do |format|
        format.html { redirect_to document_document_permissions_path(@document), notice: 'Permissão revogada com sucesso!' }
        format.json { render json: { success: true, message: 'Permissão revogada com sucesso!' } }
      end
    else
      respond_to do |format|
        format.html { redirect_to document_document_permissions_path(@document), alert: 'Erro ao revogar permissão.' }
  ...
  def ensure_can_manage_permissions
    unless @document.user_can_edit?(current_user)
      respond_to do |format|
        format.html { redirect_to @document, alert: 'Você não tem permissão para gerenciar permissões deste documento.' }
