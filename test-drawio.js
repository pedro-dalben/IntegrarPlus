  ...
  try {
    ...
    if (iframeExists) {
      ...
      if (saveButtonExists) {
        ...
        const hasStimulusLogs = consoleLogs.some(log => 
          log.includes('DrawioController connected') || 
          log.includes('Draw.io iframe loaded')
        );
        ...
        const hasSaveLogs = consoleLogs.some(log => 
          log.includes('Save button clicked') || 
          log.includes('Sending message to iframe')
        );
