...
namespace :emails do
  ...
  task diagnostic: :environment do
    ...
    puts "  Host: #{ENV.fetch('OCI_SMTP_HOST', nil)}"
    puts "  Port: #{ENV.fetch('OCI_SMTP_PORT', nil)}"
    puts "  Domain: #{ENV.fetch('MAIL_DOMAIN', nil)}"
    puts "  From: #{ENV.fetch('MAIL_FROM', nil)}"
    ...
    puts "  Host: #{ENV.fetch('APP_HOST', nil)}"
    puts "  Protocol: #{ENV.fetch('APP_PROTOCOL', nil)}"
  ...
  task :test, [:email] => :environment do |_t, args|
    email = args[:email] || ENV.fetch('TEST_EMAIL', nil)
